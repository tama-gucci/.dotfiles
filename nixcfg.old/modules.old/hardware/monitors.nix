...
let
  ...
  # Format: name,resolution@rate,position,scale[,transform,N][,vrr,N][,bitdepth,10]
  # HDR requires 10-bit color depth
  toHyprlandMonitor = mon:
    if mon.enabled then
      let
        ...
        # HDR requires 10-bit color depth - Hyprland specific
        hdrPart = optionalString mon.hdr ",bitdepth,10";
      in
        base + transformPart + vrrPart + hdrPart
    else
      "${mon.name},disable";
  
  # Generate Niri output block
  # Note: Niri VRR is a simple on/off toggle (no levels like Hyprland)
  # Note: HDR is not currently supported in Niri
  toNiriOutput = mon:
    if mon.enabled then
      ''
        output "${mon.name}" {
            ...
            ${optionalString (mon.vrr > 0) "variable-refresh-rate on-demand"}
        }
      ''
    else
      ''
...
in
{
  options.modules.monitors = {
    ...
    hdrEnabled = mkOption {
      ...
      description = ''
        Whether any monitor has HDR enabled.
        Note: HDR is only supported by Hyprland. Niri does not currently support HDR.
      '';
    };
    ...
    vrrEnabled = mkOption {
      type = types.bool;
      readOnly = true;
      default = any (m: m.vrr > 0 && m.enabled) cfg.displays;
      description = ''
        Whether any monitor has VRR (Variable Refresh Rate) enabled.
        Works with both Hyprland and Niri compositors.
      '';
    };
  };
